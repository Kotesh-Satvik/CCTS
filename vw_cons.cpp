...
exponential_distribution<double> distr(l);
	...
	for(int curTrans = 0; curTrans<n ; curTrans ++){
		...
		do{
			...
			for(int i=0; i<randIters; i++){
                ...
                // cout <<" Transaction " << t->id << " reads from " << randInd  << " a value " << locVal << "\n";
                ...
                locVal += randVal;
                // cout <<" Transaction " << t->id << " writes to " << randInd  << " a value " << locVal << "\n";
				...
				pthread_mutex_unlock(&lock5);
                float randTime = distr(generator);
